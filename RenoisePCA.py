...
def renoisePCA(image, regionsize, blocksize, sigma):
    ...
    latent_per_block = np.empty((windows_dataset.shape[0], windows_dataset.shape[-1]))
    for i in range(windows_dataset.shape[-1]):
        latent_per_block[..., i] = pca_svd_latent(windows_dataset[..., i])
